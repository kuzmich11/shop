...
class Cart {
    ...
    show() {
        let clickButton = document.querySelector('.btn-cart');
        clickButton.addEventListener('click', function(event) {
            let cartShow = document.querySelector('.cart');
            cartShow.classList.toggle('cart-none');
        })
...
class CartItem {
    ...
    render() {
        return `<div class="cart-item" data-id="${this.id}">
                    <img width="100px"  src="${this.img}" alt="Some img">
                    <div class="cart-text">
                        <p>${this.title}</p>
                        <p>Quantity: ${this.quantity}</p>
                        <p class="cart-text">$${this.price}</p>
                    </div>
